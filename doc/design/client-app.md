...
```text
顶部来源 / 同步 / 导入 / 运行环境摘要
逾期、今天截止、待处理、任务总数、备忘连续统计条
attention_items 风险优先列表
finance_overview 收支、预算、分类占比和洞察
七日支出趋势
recent_activity 备忘、任务和流水混合活动流
```

宽屏从 1080px 开始使用 7:5 主次两栏，窄屏保持单列滚动和下拉刷新。首页使用共享 `WorkspacePanel` 组织同类信息，通过分隔线而不是独立 Card 承载连续内容。业务状态色统一来自主题的 `LiflySemanticColors`，页面不得硬编码红、橙、绿或主题包具体色值。
