...
Web dashboard 使用与 A 方向原型一致的 218px 自定义侧栏，折叠后为 64px；桌面 compact 继续使用紧凑 `NavigationRail`。两者共享首页、备忘、AI、记账、任务五个核心入口，不把手机底栏直接放大。
...
Web 首页是 `HomeOverview` 的真实消费页面，不在 Widget 中写入演示数据或重新计算长期聚合规则。v0.8.2 已落地：

```text
68px 左对齐“今天”顶栏，显示日期、搜索和日程入口
attention_items 最多四条组成今日处理队列
finance_overview 以支出、预算余量、主要分类三条比例行展示
recent_activity 只提取最近备忘，继续未完成思路
带 occurred_at 的 attention_items 组成右侧今日时间线
sync_summary 在时间线底部提供真实数据状态
```

Web 内容宽度不低于 860px 时使用主工作区与日程右栏，低于该宽度时日程移到主内容下方；主工作区宽度不低于 760px 时，本月收支和最近备忘并排。首页严格使用 A 方向原型的连续行、上下边线和独立日程栏，不显示上一版的状态总览面板、连续指标条、七日柱状图或混合活动面板。手机端继续使用既有 `HomeDashboardView`，本轮不改变手机布局。业务状态色统一来自主题的 `LiflySemanticColors`。
